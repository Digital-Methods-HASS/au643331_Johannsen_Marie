...
Western_Europe %>% 
  rename(Country = Entity) -> Western_Europe
```
...
ggplot(data = Western_Europe) + 
  ...
  geom_line()+
  labs(x = "Year", y = "Number of homicides per 100k people")
```
...
In the early years, the homicide rate was higher and the duration of reigns shorter. 

```{r eval = TRUE, warning=FALSE}
Danish_kings <- read_csv("data/Konger.csv")
Danish_kings <- Danish_kings %>% 
  mutate("midyear" = `End reign`-(`End reign` - `Start reign`)/2) %>% 
  mutate("duration_reign" = `End reign` - `Start reign`)
ggplot(Danish_kings)+
  aes(x = midyear, y = duration_reign)+
  geom_smooth()+
  labs(x="Midyear of rulers reign", y="Duration of reign", title = "Reign of Danish Rulers")
```
...
The data shows a decreasing amount of homicides over time in all the countries listed, which would suggest that we are more civilized today, since a characteristic of a civilized society would be less involvement in violent acts. The data of the danish kings show that the duration of reign is steadily increasing for the most part, however, this adds little to answering the question of a more civilized society, since there can be a vast number of reasons behind the end of a reign. On the other hand, the end of a reign is most likely due to either violence or sickness, and the decrease of both these factors would also suggest for a more civilized society today. 