...
date: 'created on 6 Oktober 2023 and updated `r format(Sys.time(), "%d %B, %Y")`'
...
### Getting the GoT report:

Before reaching this step, I have loaded 6 packages into R\
- tidyverse\
- here\
- tidytext\
- pdftools\
...
Next, we load the GoT-book into R by finding it in our data-folder.
...
-   How cool to extract text out of a PDF!
...
Example: If we just want to get text from a single page, e.g. Page 9, we can do this as follows:

```{r single-page, include=TRUE}
got_p9 <- got_text[9]
got_p9
```

See how that compares to the text in the PDF on Page 9. What has pdftools added and where?\
- We can compare this to the actual pdf-file and see, that R adds a lot of backslashes followed by an 'n', which corresponds to linebreaks in the pdf-text.
...
Wait, so "lord" is showing up in NRC lexicon as both positive AND negative? Let's check:
...
-   Most common meaningful words for the GoT-set after filtering away the stopwords are lord, ser, eyes, hand and names of the characters.

-   I chose to investigate the word "lord", which is very ambigious, as it is both positive and negative and associated both trust and disgust.
